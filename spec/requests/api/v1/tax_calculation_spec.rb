...
RSpec.describe 'GET /api/v1/tax_calculation', type: :request do
  ...
  let!(:bracket2) { TaxBracket.create!(currency: currency, lower_cents: 1_560_000,  upper_cents: 5_350_000,  rate: BigDecimal('0.175')) }
  let!(:bracket3) { TaxBracket.create!(currency: currency, lower_cents: 5_350_000,  upper_cents: 7_810_000,  rate: BigDecimal('0.30')) }
  let!(:bracket4) { TaxBracket.create!(currency: currency, lower_cents: 7_810_000,  upper_cents: 18_000_000, rate: BigDecimal('0.33')) }
  let!(:bracket5) { TaxBracket.create!(currency: currency, lower_cents: 18_000_000, upper_cents: nil,        rate: BigDecimal('0.39')) }
  ...
  context 'happy path' do
    ...
    it 'returns correct tax for 10,000' do
      get '/api/v1/tax_calculation', params: { income: 10000 }
      expect(response).to have_http_status(:ok)
      body = json_body
      # 10,000 * 10.5% = 1,050.00
    ...
    it 'returns correct tax for 35,000' do
      get '/api/v1/tax_calculation', params: { income: 35000 }
      expect(response).to have_http_status(:ok)
      body = json_body
      # 15,600 * 10.5% = 1,638.00
      # (35,000 - 15,600) * 17.5% = 3,395.00
      # Total tax = 1,638 + 3,395.00 = 5,033.00
      expect(body['total_tax']).to eq('5033.00')
    end

    it 'returns correct tax for 100,000' do
      get '/api/v1/tax_calculation', params: { income: 100000 }
      expect(response).to have_http_status(:ok)
      body = json_body
      # 15,600 * 10.5% = 1,638.00
      # (53,500 - 15,600) * 17.5% = 6,632.50
      # (78,100 - 53,500) * 30% = 7,380.00
      # (100,000 - 78,100) * 33% = 7,227.00
      # Total tax = 1,638 + 6,632.50 + 7,380 + 7,227 = 22,877.50
      expect(body['total_tax']).to eq('22877.50')
    end

    it 'returns correct tax for 220,000' do
      get '/api/v1/tax_calculation', params: { income: 220000 }
      expect(response).to have_http_status(:ok)
      body = json_body
      # 15,600 * 10.5% = 1,638.00
      # (53,500 - 15,600) * 17.5% = 6,632.50
      # (78,100 - 53,500) * 30% = 7,380.00
      # (180,000 - 78,100) * 33% = 34,377.00
      # (220,000 - 180,000) * 39% = 15,600.00
      # Total tax = 1,638 + 6,632.50 + 7,380 + 34,377 + 15,600 = 64,877.50
      expect(body['total_tax']).to eq('64877.50')
    end
  end
